...
- **Where it lives:** `addons/titles.sw` next to `scripts/` is found automatically;
  anywhere else needs [`--addon-path`](/1.1.0/reference/cli) at check/compile time and the
...
NPCs; both have since [graduated](/1.1.0/reference/holograms) to first-class constructs, but
the [raw packets reference](/1.1.0/reference/packets) still covers the packet techniques the
NPC addon used.
