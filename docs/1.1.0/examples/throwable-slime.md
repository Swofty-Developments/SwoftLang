...
`on dispense of slime ball` maps straight onto the [`BlockDispense`](/1.1.0/reference/dispensers#the-blockdispense-event) event: it hands the handler the block's `location`, its facing `direction`, and the `item` about to leave. Cancel the vanilla eject and hand the position and direction to the *same* `fly_slime` task the thrower uses — one bouncing ball, two ways to launch it. The .sk's `~ vector from facing` offset and metadata tags fold into that shared task.
