...
`ServerPing` fires for each server-list ping, with `motd`, `online`, and `max` all
writable — dynamic MOTDs and fake player counts, per ping:
...
1. Compiles/loads every script (via `swoftc` or [sidecars](./cli#sidecars)),
   resolving [imports](/1.1.0/libraries/) as it goes.
...
`storage` is the `server`-block sibling for [persistent variables](/1.1.0/guide/persistence):
