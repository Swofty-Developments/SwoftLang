...
[Step 07](/1.8.0/guide/options) applies), and worlds expose time control:
...
[Step 16](/1.8.0/guide/ship-it#worlds).
...
[async](/1.8.0/guide/async#what-runs-where). You never write dispatch code — the table does it.
...
| `stats`, `tags` | — | custom-item namespaces — [Step 12](/1.8.0/guide/items-and-mobs) |
...
Listed per event in the [event table](/1.8.0/guide/events#the-event-table). Later steps add
`Mob` ([Step 12](/1.8.0/guide/items-and-mobs)) and `Display` ([Step 13](/1.8.0/guide/modules))
