...
the time async code runs ([why](/1.8.0/guide/events#cancelling-events)):
...
the result is already there. The `Profile` here is a [struct](/1.8.0/reference/structs); futures
pair naturally with struct loads and [persistence](/1.8.0/guide/persistence) reads, which is
...
- **Cancellation** on reload or shutdown. Every in-flight task is torn down; `await`
  ...
  case you handle.
- **Missing data is not a failure.** "No profile found" is an [`Optional`](/1.8.0/guide/options),
...
reference](/1.8.0/reference/futures#error-model) for the full rationale.
...
- **Touching the world** — teleporting, setting health, changing held items — must
  happen on the tick thread. Every [property](/1.8.0/guide/properties#threading) knows its
