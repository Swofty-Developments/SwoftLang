...
running on a background [virtual thread](/1.9.0/guide/async#what-runs-where) and the future is
...
producing, consuming, and combining futures; the [Async guide](/1.9.0/guide/async#futures) is
the narrative introduction.

`Future<T>` is generic and PascalCase like [`List<T>`](/1.9.0/reference/collections) or
[`Optional<T>`](/1.9.0/guide/options). You never construct one directly — `spawn` and `async { }`
produce them.
...
callables](/1.9.0/guide/async) are spawnable — spawning a plain sync function is an error, the
...
[color](/1.9.0/guide/async#the-coloring-rules); on the tick thread you detach into an `async { }`
...
thread until the future resolves (cheap — the same parking [`wait`](/1.9.0/guide/async) does) and
...
hopping back to the tick thread](/1.9.0/guide/async#what-runs-where),
[scope snapshots](/1.9.0/guide/async#scope-snapshots), `halt` killing only its own task — is
...
today does *outbound* IO: SwoftLang has inbound [`api`](/1.9.0/reference/http-api) routes but no
...
- **Missing data is not a future failure** — model it as [`Optional<T>`](/1.9.0/guide/options).
...
- **A player or entity gone mid-load is not a future failure** — the value succeeded.
  ...
  [stale-player](/1.9.0/guide/async#stale-players) guard.
...
- [Async guide](/1.9.0/guide/async#futures) — the narrative walkthrough with a combined example
- [Structs](/1.9.0/reference/structs) and [Persistence](/1.9.0/guide/persistence) — where the
  "not-ready-yet" values (loaded records) typically come from
- [Options](/1.9.0/guide/options) — how missing data is modeled, *not* as a future failure
