...
`target` is an [`Optional`](/1.9.0/guide/options) because there may be nothing selected this
...
exactly as they are in any other [tick-colored](/1.9.0/guide/async#the-coloring-rules) code:
...
the `await` [auto-hops back](/1.9.0/guide/async#what-runs-where) onto the tick thread. The `on_tick`
...
- [Futures](./futures) and the [Async guide](/1.9.0/guide/async) — `spawn`/`async` + `await`, the
  tick-thread detach pattern goals use for slow data
- [Entities](./entities) — the property surface every `mob` composes
- [Options](/1.9.0/guide/options) — how `target` models "nothing selected"
