...
compile as one unit; [imports](/1.9.0/libraries/) are resolved transitively and the whole
...
ignore it. See [the module system](/1.9.0/libraries/#the-import-system) for resolution
details.
...
[persistence](/1.9.0/guide/persistence) store is not part of the teardown: persistent variables
stay in memory across the reload (no flush-and-reload round trip), so counters, homes,
guilds, and [reactive structs](/1.9.0/reference/structs#reactive) carry straight over. After the
