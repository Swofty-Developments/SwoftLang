...
the type system, and it's [Step 07](/1.9.0/guide/options#functions-that-might-not-return-a-value).
...
[brace-free body form](/1.9.0/guide/control-flow#brace-free-bodies) from Step 05.)
...
[Step 13](/1.9.0/guide/modules#lambdas-across-modules) possible.
...
[Step 07](/1.9.0/guide/options) does not survive into a lambda body — the body may run long
after the check, so re-check or use `otherwise` inside.

### Async lambdas

`async function(...)` gives a lambda the right to `wait`. The coloring rules
([Step 09](/1.9.0/guide/async)) match named functions: call it directly only from async code;
