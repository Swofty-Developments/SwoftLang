...
[**persistent**](/1.5.0/guide/persistence) — provided every one of its fields is a persistable
...
The fix is the familiar one from [persistence](/1.5.0/guide/persistence): store the uuid
...
- **The field's type picks the vocabulary.** `a: Player` gets [Player](/1.5.0/reference/events#player)
  handlers (`on_death`, `on_chat`, …); a `boss: Mob` field would get [Mob](/1.5.0/reference/events#mob)
  handlers instead. The receiver method table is exactly the one from
  [Receivers & Events](/1.5.0/reference/events).
...
[most-specific wins](/1.5.0/reference/events#overriding-a-base-receiver), unchanged), then every
...
it. [`call original method`](/1.5.0/reference/events#overriding-a-base-receiver) keeps its
