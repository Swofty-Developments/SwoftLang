...
[lambda](/1.5.0/guide/functions#inline-functions-lambdas):
...
from within (distinct from [`halt`](/1.5.0/guide/control-flow), which ends the current task);
...
distinct from [tags](/1.5.0/reference/mobs#tags) whose values are `Schedule` handles. You bind a
...
so it composes with the [option operators](/1.5.0/guide/options):
...
| `Mob` | `mob` inside [`on_spawn`](/1.5.0/reference/mobs), a `Mob` value elsewhere |
| `Entity` | any [`Entity`](/1.5.0/reference/entities) value |
| `Npc` | `npc` inside an [npc](/1.5.0/reference/npcs) handler (`on_click`, `on_tick`) |
| `Hologram` | `hologram` inside a [hologram](/1.5.0/reference/holograms) handler |
...
block (see [`remove block at`](/1.5.0/reference/blocks#placing-and-reading-blocks)) cancels every
...
| one task that must also *react* (waits interleaved with logic) | `spawn` an [async function](/1.5.0/guide/async) |
...
The first-class [`npc`](/1.5.0/reference/npcs) construct's `look_at_players` head tracking is
