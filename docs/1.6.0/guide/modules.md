...
error it would be in one file, [optional](/1.6.0/guide/options) types flow through, and
[async coloring](/1.6.0/guide/async) is enforced across module calls.
...
[Libraries](/1.6.0/libraries/) section documents each API):
...
[`hologram`](/1.6.0/reference/holograms) and [`npc`](/1.6.0/reference/npcs) constructs. Using an
...
[first-class values](/1.6.0/guide/functions#inline-functions-lambdas): you can hand a module a
lambda and let it call you back. The [abilities addon](/1.6.0/libraries/abilities)'s handler
API is exactly that —
