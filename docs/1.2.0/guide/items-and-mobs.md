...
items, so [Step 07](/1.2.0/guide/options) discipline applies:
...
14](/1.2.0/guide/rpg-items) walks the full Hypixel-style pattern, and the
[reference](/1.2.0/reference/items) has every key.
:::
...
A custom mob is an [entity](/1.2.0/reference/entities), so the shared entity properties and
...
Server-wide, the base [`Mob`](/1.2.0/reference/events#mob) receiver's `on_spawn`, `on_death`, and
...
That's a kill tracker in ten lines — persistence from [Step 10](/1.2.0/guide/persistence),
optionals from [Step 07](/1.2.0/guide/options), and content from this one. Next: packaging
