...
suggests the fix ([Step 08](/1.2.0/guide/properties)). `broadcast "..."` is shorthand for
`send "..." to all`.
...
Which parameters are writable is fixed per method (the [receiver tables](/1.2.0/reference/events)
...
[Receivers & Events reference](/1.2.0/reference/events). The headline `Player` methods this guide
leans on:
...
Custom [items and mobs](/1.2.0/guide/items-and-mobs) get their own methods on top of the base
receivers; `Server { on_list_ping(status) }` powers dynamic MOTDs in
[Step 16](/1.2.0/guide/ship-it#a-dynamic-motd).
...
([the async model](/1.2.0/guide/async#the-coloring-rules) is Step 09):
