...
Registers `handler` to run when a player uses the [custom item](/1.2.0/reference/items) with id
`item_id`. `filter` is `"left"`, `"right"`, or `"any"`, kept for parity with the item
[`on_click`](/1.2.0/reference/items#on-click) sugar. Registering again for the same id + filter
replaces the previous handler. The handler receives the acting `Player` and the `Item`
in hand — read its [tags](/1.2.0/reference/items#tags-the-nbt-api) to drive stat formulas:
...
- **A module-level association chain** ([module vars](/1.2.0/guide/modules#module-vars)) stores
...
- **A self-counted tick clock.** An `every 1 ticks` block increments a module counter, so
  ...
  wall-clock builtin.
- **A single generic listener.** One base [`Item.on_use`](/1.2.0/reference/events#item) method
...
separation, use the item declaration's [`on_click(<side>)`](/1.2.0/reference/items#on-click)
