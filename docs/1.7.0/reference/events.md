...
A third kind of receiver lives on a [struct](/1.7.0/reference/structs). Mark a struct field
...
are durable and self-cleaning — see [Reactive fields](/1.7.0/reference/structs#reactive) for the
...
not block a tick, following the usual [async coloring](/1.7.0/guide/async) rules. Cancellation is
...
[`command "..." { }`](/1.7.0/guide/commands) declarations are not events — they define the
