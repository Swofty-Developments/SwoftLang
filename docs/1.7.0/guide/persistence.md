...
The whole `Item` — enchantments, custom name, [custom-item](/1.7.0/reference/items) identity —
survives the round trip, so a saved kit comes back exactly as it went in.

### Persistent structs

A [struct](/1.7.0/reference/structs) whose fields are *all* persistable types is itself
...
Storing a struct persistently also makes any [`@EventReceiver`](/1.7.0/reference/structs#reactive)
field on it *live* — reachability from a persistent root is what activates a reactive
struct's handlers. See [Structs → persistent state](/1.7.0/reference/structs#persistent) for the
full treatment.
...
This is the same [totality](/1.7.0/reference/structs) that makes every persistent read return a
...
for missing values by hand, which is exactly what [Step 07](/1.7.0/guide/options) abolished.
:::
