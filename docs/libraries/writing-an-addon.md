...
- **Where it lives:** `addons/titles.sw` next to `scripts/` is found automatically;
  anywhere else needs [`--addon-path`](/reference/cli) at check/compile time and the
...
NPCs; both have since [graduated](/reference/holograms) to first-class constructs, but
the [raw packets reference](/reference/packets) still covers the packet techniques the
NPC addon used.
