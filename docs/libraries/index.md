...
overridable with [`swoftc --addon-path <dir>`](/reference/cli). A failed resolution
...
[bundle JSON](/reference/json-ast#bundles). Import graphs must be acyclic:
...
[`persistent`](/guide/persistence) for data that must survive restarts).
...
the updates through a single [schedule](/reference/schedulers) or a persistent
