...
The `npcs` addon has been replaced by the built-in [`npc`](/reference/npcs) declaration:
...
one declaration the compiler checks. See the [first-class NPC reference](/reference/npcs)
for the construct and its statements.

The retired addon was ~400 lines of SwoftLang over the [raw packet](/reference/packets)
...
[raw packets reference](/reference/packets) cover the packet and registry patterns — but
