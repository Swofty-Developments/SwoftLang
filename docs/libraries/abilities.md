...
Registers `handler` to run when a player uses the [custom item](/reference/items) with id
`item_id`. `filter` is `"left"`, `"right"`, or `"any"`, kept for parity with the item
[`on_click`](/reference/items#on-click) sugar. Registering again for the same id + filter
replaces the previous handler. The handler receives the acting `Player` and the `Item`
in hand — read its [tags](/reference/items#tags-the-nbt-api) to drive stat formulas:
