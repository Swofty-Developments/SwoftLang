...
[storage backend](/1.4.0/guide/persistence#the-storage-block). You query it with
...
because [persistents](/1.4.0/guide/persistence) are keyed by UUID and accept either an
...
That's the full picture: [`offline_player`](/1.4.0/reference/offline-players#builtins) for the
...
all null-free. The [reference page](/1.4.0/reference/offline-players) has the complete property
and builtin tables.
