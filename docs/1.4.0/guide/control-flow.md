...
[Step 07](/1.4.0/guide/options) shows how far that goes.
...
open-ended loop, you want an [async task](/1.4.0/guide/async) with a `wait` in it — that parks
...
script run, not just the function (use [`return`](/1.4.0/guide/functions#return) for that).
Inside an async task, `halt` kills *that task only*
([details](/1.4.0/guide/async#halt-in-a-task)).
