...
The `npcs` addon has been replaced by the built-in [`npc`](/1.4.0/reference/npcs) declaration:
...
one declaration the compiler checks. See the [first-class NPC reference](/1.4.0/reference/npcs)
for the construct and its statements.

The retired addon was ~400 lines of SwoftLang over the [raw packet](/1.4.0/reference/packets)
...
[raw packets reference](/1.4.0/reference/packets) cover the packet and registry patterns — but
