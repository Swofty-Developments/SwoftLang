...
the compiler suggests the fix ([Step 08](/1.3.0/guide/properties)). `broadcast "..."` is
...
[receiver tables](/1.3.0/reference/events) mark them `rw`) — `message` is writable, `player` is
not.
...
[Receivers & Events reference](/1.3.0/reference/events). The headline `Player` methods this guide
...
Custom [items and mobs](/1.3.0/guide/items-and-mobs) get their own methods on top of the base
receivers; `Server { on_list_ping }` powers dynamic MOTDs in
[Step 16](/1.3.0/guide/ship-it#a-dynamic-motd).
...
([the async model](/1.3.0/guide/async#the-coloring-rules) is Step 09):
