...
An `item` gives you identity, appearance, WYSIWYG [lore](/1.3.0/reference/items#lore), and a
nested [NBT tag API](/1.3.0/reference/items#tags-the-nbt-api). Everything a Hypixel-style item
...
your stats — and your own tier scheme — as [tags](/1.3.0/reference/items#tags-the-nbt-api),
...
The four vanilla [rarities](/1.3.0/reference/items#rarity) tint the name; anything richer —
...
reading `0`. This is the [options](/1.3.0/guide/options) discipline reaching all the way into
...
while it recharges. The [`abilities`](/1.3.0/libraries/abilities) addon provides it in ~50
...
[lambda](/1.3.0/guide/functions#inline-functions-lambdas), so a `with_cooldown`-wrapped
...
a single item, the [`on_click`](/1.3.0/reference/items#on-click) sugar skips the addon entirely;
