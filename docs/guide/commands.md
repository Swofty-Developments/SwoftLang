...
`<lime>`-style color tags are covered in [Step 04](/guide/variables-and-types).
...
([Step 04](/guide/variables-and-types#type-checks-is-a-is-not-a)).
...
- `permission` — the sender must hold this node or the command is refused (you'll grant
  nodes in [Step 16](/guide/ship-it#permissions)).
...
`args.target.name` go through the [property system](/guide/properties) and are checked
...
[Step 05](/guide/control-flow); first, the other way scripts get triggered.
