...
error it would be in one file, [optional](/guide/options) types flow through, and
[async coloring](/guide/async) is enforced across module calls.
...
[Libraries](/libraries/) section documents each API):
...
[`hologram`](/reference/holograms) and [`npc`](/reference/npcs) constructs. Using an
...
[first-class values](/guide/functions#inline-functions-lambdas): you can hand a module a
lambda and let it call you back. The [abilities addon](/libraries/abilities)'s handler
API is exactly that —
