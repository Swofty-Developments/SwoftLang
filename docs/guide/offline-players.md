...
because [persistents](/guide/persistence) are keyed by UUID and accept either an
...
That's the full picture: [`offline_player`](/reference/offline-players#builtins) for the
...
all null-free. The [reference page](/reference/offline-players) has the complete property
and builtin tables.
