...
[async](/guide/async#what-runs-where). You never write dispatch code — the table does it.
...
| `stats`, `tags` | — | custom-item namespaces — [Step 12](/guide/items-and-mobs) |
...
Listed per event in the [event table](/guide/events#the-event-table). Later steps add
`Mob` ([Step 12](/guide/items-and-mobs)) and `Display` ([Step 13](/guide/modules))
