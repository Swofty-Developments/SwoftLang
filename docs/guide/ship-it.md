...
Back in [Step 02](/guide/commands#permission-and-description) you gated commands with
...
- `polar_storage_loader(<backend>)` — polar bytes in a
  [Step 10 storage backend](/guide/persistence#the-storage-block), keyed by world name.
...
[Step 09](/guide/async): a slow body can't stall the game, and every scheduled task is
...
[syntax cheatsheet](/reference/syntax-cheatsheet) open, raid the
[examples](/examples/) for full ported plugins, and go ship something.
