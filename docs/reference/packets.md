...
The first-class [`npc`](/reference/npcs) construct's runtime is built entirely on this
...
[`npc`](/reference/npcs) runtime tells left clicks from right clicks.
...
| fake players / NPCs | the [`npc`](/reference/npcs) construct — first-class — or packets directly |
