...
[**persistent**](/guide/persistence) — provided every one of its fields is a persistable
...
The fix is the familiar one from [persistence](/guide/persistence): store the uuid
...
- **The field's type picks the vocabulary.** `a: Player` gets [Player](/reference/events#player)
  handlers (`on_death`, `on_chat`, …); a `boss: Mob` field would get [Mob](/reference/events#mob)
  handlers instead. The receiver method table is exactly the one from
  [Receivers & Events](/reference/events).
...
[most-specific wins](/reference/events#overriding-a-base-receiver), unchanged), then every
...
it. [`call original method`](/reference/events#overriding-a-base-receiver) keeps its
