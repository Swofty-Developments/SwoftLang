package net.swofty.ui;

import net.minestom.server.entity.PlayerSkin;
import net.swofty.ASTExecutor;
import net.swofty.ScriptError;
import net.swofty.model.SkinSpecModel;
import net.swofty.props.Coercions;

/**
 * Builtin tablist skin palette. The six base64 texture + Mojang-signature
 * pairs are copied verbatim from the SkyBlock TablistSkinRegistry; they
 * render as flat colored heads used as row/header icons.
 */
public final class TablistSkins {
    public static final PlayerSkin GREEN = new PlayerSkin(
            "",
            "");
    public static final PlayerSkin GRAY = new PlayerSkin(
            "",
            "");
    public static final PlayerSkin CYAN = new PlayerSkin(
            "",
            "");
    public static final PlayerSkin BLUE = new PlayerSkin(
            "",
            "");
    public static final PlayerSkin PURPLE = new PlayerSkin(
            "",
            "");
    public static final PlayerSkin ORANGE = new PlayerSkin(
            "",
            "");

    private TablistSkins() {
    }

    public static PlayerSkin builtin(String name) {
        return switch (name) {
            case "green" -> GREEN;
            case "gray" -> GRAY;
            case "cyan" -> CYAN;
            case "blue" -> BLUE;
            case "purple" -> PURPLE;
            case "orange" -> ORANGE;
            default -> throw new ScriptError("unknown tablist skin '" + name
                    + "' (valid: green, gray, cyan, blue, purple, orange)");
        };
    }

    /**
     * Resolve a skin spec: builtin palette name, skin of a player
     * expression, or a custom texture/signature pair; null spec = gray
     */
    public static PlayerSkin resolve(SkinSpecModel spec, ASTExecutor executor) {
        if (spec == null) {
            return GRAY;
        }
        return switch (spec.kind()) {
            case BUILTIN -> builtin(spec.builtinName());
            case PLAYER -> {
                Object value = executor.evaluateExpression(spec.playerExpr());
                if (!(value instanceof net.minestom.server.entity.Player player)) {
                    throw new ScriptError("skin of expects a player, got: "
                            + executor.getDisplayString(value));
                }
                PlayerSkin skin = player.getSkin();
                yield skin != null ? skin : GRAY;
            }
            case CUSTOM -> new PlayerSkin(
                    (String) Coercions.toStringValue(executor.evaluateExpression(spec.texture())),
                    (String) Coercions.toStringValue(executor.evaluateExpression(spec.signature())));
        };
    }
}
