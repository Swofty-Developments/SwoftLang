// scheduler v2: 'stop', 'run' counter, 'repeat N times', named schedules, is_running

// named top-level 'every' that stops itself after 5 runs
...
command "flash" {
    execute {
        ...
        // named schedule handle that self-cancels at run 10
        set h to schedule every 1 seconds as "counter" {
            if run >= 10 stop
        }
        // is_running by handle and by name
