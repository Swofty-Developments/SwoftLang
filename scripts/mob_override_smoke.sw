...
// `call original method` inside an overriding mob handler chains into the base
// Mob.<method> and that most-specific-wins suppresses the base event otherwise.
//
...
// specific custom declaration overrides it. Uses `mob.custom_id`, so any run
// of this body (directly, or via an override's `call original method`) proves
// the acted-on mob was threaded through.
Mob {
    on_hit {
        if attacker is a Player {
            send "TRACE=BASE_A:${mob.custom_id}" to attacker
        }
    }
}
